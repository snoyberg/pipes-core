...
liftF c = Free (fmap return c) (return . throw)
...
catchP (Free c h1) h2 = Free c $ \e ->
  h1 e >>= \p -> return $ catchP p h2
catchP (Throw e) h = Free (M (h e) Strict) (return . throw)
...
compose (M m Strict) _ = AdvanceFirst (lift_ Strict m)
...
p1 >+> p2 = case (p1, p2) of
  (Free c1 h1, Free c2 h2) -> case compose c1 c2 of
    AdvanceFirst comp -> catchP comp (liftH h1) >>= \p1' -> p1' >+> p2
    AdvanceSecond comp -> catchP comp (liftH h2) >>= \p2' -> p1 >+> p2'
    AdvanceBoth p1' p2' -> p1' >+> p2'
  (Free _ h, Throw e) ->
    lift_ Strict (h e) >>= \p1' -> p1' >+> p2
  (Throw e, Free _ h) ->
    lift_ Strict (h e) >>= \p2' -> p1 >+> p2'
  (Free _ h, Pure _) ->
    lift_ Strict (h (E.toException BrokenDownstreamPipe)) >>= \p1' -> p1' >+> p2
  (Pure _, Free _ h) ->
    lift_ Strict (h (E.toException BrokenUpstreamPipe)) >>= \p2' -> p1 >+> p2'
  (_, Throw e) -> throw e
  (_, Pure r) -> return r
  where
    liftH h e = h e >>= \p' -> return (return p')
...
runPurePipe (Free c h) = stepPipe try c >>= \x -> case x of
  Left e   -> h e >>= runPurePipe
  Right p' -> runPurePipe p'
